...
dt_datatable_wrapper(applications)
```
...
### Previous credit applications data aggregation

```{r}
# Count the total number of previous applications for each SK_ID_CURR
total_applications <- count(previous_applications, sk_id_curr, name = 'total_num_previous_apps')

# Aggregate accepted applications data
accepted_agg <- accepted_applications %>% 
  group_by(sk_id_curr) %>% 
  summarise(
    # Count the total number of accepted applications
    total_accepted_apps = n(),  
    # Calculate the sum of accepted credit amounts
    total_accepted_credit = sum(amt_credit, na.rm = TRUE),  
    # Calculate the mean downpayment rate for accepted applications
    accepted_median_downpayment_rate = median(rate_down_payment, na.rm = TRUE),  
    # Calculate the proportion of accepted applications with downpayment
    ...
    # Find the maximum days to the last due date for accepted applications
    accepted_max_days_first_due = max(days_first_due, na.rm = FALSE)  
  )

# Aggregate rejected applications data
rejected_agg <- rejected_applications %>% 
  group_by(sk_id_curr) %>% 
  summarise(
     # Count the total number of rejected applications
    total_rejected_apps = n(), 
    # Calculate the sum of rejected credit amounts
    total_rejected_credit = sum(amt_credit, na.rm = TRUE),  
    # Calculate the mean downpayment rate for rejected applications
    rejected_median_downpayment_rate = median(rate_down_payment, na.rm = TRUE),  
    # Calculate the proportion of rejected applications with downpayment
...
previous_applications_agg <- merge(total_applications, accepted_agg, 
  ...
  mutate(
    ...
    # Calculate the acceptance rate of previous applications
...
# Merge the 'applications' and 'previous_credits_agg' dataframes by 'sk_id_curr', keeping all rows from 'applications'
...
# Replace NAs in aggregated columns with zeros. In these datasets, NA means no data
cols2replace <- c(colnames(credit_agg), 
                  colnames(installments_agg),
                  colnames(previous_credits_agg), 
                  colnames(previous_applications_agg)) %>% unique()
...
write.csv(df, 'data/all_data_merged.csv', row.names = FALSE)
